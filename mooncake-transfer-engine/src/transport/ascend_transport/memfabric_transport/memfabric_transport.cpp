...
Status MemFabricTransport::submitTransfer(Transport::BatchID batch_id, const std::vector<TransferRequest> &entries)
{
    auto count = entries.size();
    auto &batch_desc = *((BatchDesc *)(batch_id));
    if (batch_desc.task_list.size() + count > batch_desc.batch_size) {
        LOG(ERROR) << "MemFabricTransport: Exceed the limitation of current batch's "
                      "capacity";
        return Status::InvalidArgument(
                "MemFabricTransport: Exceed the limitation of capacity, batch id: " +
    ...
    smem_bm_copy_type t = entries[0].opcode == TransferRequest::READ ? SMEMB_COPY_G2L : SMEMB_COPY_L2G;
   ...
   int ret = MemFabricSmemBmDl::SmemBmCopyBatch(MemFabricSmemBmDl::GetSmemBmHandle(), &params, t, 0);
    if (ret != 0) {
...
Status MemFabricTransport::submitTransferTask(const std::vector<TransferTask *> &task_list)
{
    uint32_t count = task_list.size();

    // TODO 一次批量任务中读写混合
    // TODO 任务来自LocalHost内存
    smem_bm_copy_type t = task_list[0]->request->opcode == TransferRequest::READ ? SMEMB_COPY_G2L : SMEMB_COPY_L2G;
    std::vector<void *> sources(count);
    std::vector<void *> destinations(count);
    std::vector<uint64_t> dataSizes(count);
    for (size_t index = 0; index < task_list.size(); ++index) {
        ...
        sources[index] = request.opcode == TransferRequest::READ ? reinterpret_cast<void *>(request.target_offset)
                                                                 : request.source;
        destinations[index] = request.opcode == TransferRequest::READ ? request.source
                                                                      : reinterpret_cast<void *>(request.target_offset);
        dataSizes[index] = request.length;
    }
    smem_batch_copy_params params = {sources.data(), destinations.data(), dataSizes.data(), count};
    int ret = MemFabricSmemBmDl::SmemBmCopyBatch(MemFabricSmemBmDl::GetSmemBmHandle(), &params, t, 0);
    if (ret != 0) {
        LOG(ERROR) << "MemFabricTransport: Failed to smem bm copy batch, ret:" << ret;
    }
...
Status MemFabricTransport::getTransferStatus(Transport::BatchID batch_id, size_t task_id, Transport::TransferStatus &status)
{
    auto &batch_desc = *((BatchDesc *)(batch_id));
    const size_t task_count = batch_desc.task_list.size();
    if (task_id >= task_count) {
        return Status::InvalidArgument(
                "MemFabricTransport::getTransportStatus invalid argument, batch id: " +
                std::to_string(batch_id));
    }
...
int MemFabricTransport::install(std::string &local_server_name, std::shared_ptr<TransferMetadata> meta,
...
{
    return MemFabricSmemBmDl::SmemBmRegisterUserMem(MemFabricSmemBmDl::GetSmemBmHandle(),
                                                 ...
                                                 const std::string &location)
{
    ...
    for (const auto &item: buffer_list) {
        auto ret = MemFabricSmemBmDl::SmemBmRegisterUserMem(MemFabricSmemBmDl::GetSmemBmHandle(),
