...
int InitMemFabricBm(const std::string& master_server_entry) {
    std::string storeUrl;
    size_t colon_pos = master_server_entry.find(":");
    if (colon_pos == std::string::npos) {
        LOG(ERROR) << "Unexpected master server addr: " << master_server_entry;
        return -1;
    } else {
        std::string ip = master_server_entry.substr(0, colon_pos);
        std::string portStr = master_server_entry.substr(colon_pos + 1);
        int port = -1;
        try {
            ...
            LOG(ERROR) << "Error: Invalid argument, cannot convert '" << portStr
                       << "' to integer.";
            return -1;
        }
    ...
    if (MemFabricSmemBmDl::SmemBmInit(
        ...
        LOG(ERROR) << "Failed to init smem bm";
        return -1;
    }
    return 0;
}

int MemFabricInitSmemBm(const std::string& local_hostname,
    ...
    // init
    ret = InitMemFabricBm(master_server_entry);
    if (ret != 0) {
        return ret;
    }
    // create
