    ...
    static void InitMemFabricConfig()
    {
        ...
        if (deviceIdStr) {
            ...
            if (val >= 0 && val < 32)
                ...
                LOG(WARNING) << "Ignore value from environment variable MF_DEVICE_ID";
        }
        ...
        if (logLevelStr) {
            ...
            if (val >= 0 && val < 32)
                ...
                LOG(WARNING) << "Ignore value from environment variable MF_LOG_LEVEL";
        }
        auto dramSizeStr = std::getenv("MF_DRAM_SIZE");
        if (dramSizeStr) {
            uint64_t val = atol(dramSizeStr);
            if (val >= GB_MEM_BYTES && val <= TB_MEM_BYTES && val % (GB_MEM_BYTES) == 0)
                config_.dramSize = val;
            else
                LOG(WARNING) << "Ignore value from environment variable MF_DRAM_SIZE";
        }
